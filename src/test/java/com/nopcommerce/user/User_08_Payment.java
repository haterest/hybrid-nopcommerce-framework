...
public class User_08_Payment extends BaseTest {
    ...
    String browserName, @Optional("10") String osName, @Optional("142") String ipAddress, @Optional("9999") String portNumber) {
        ...
        billingAddressID = "billing-address-select";
    ...
    public void Payment_01_Checkout_Order_Payment_By_Cheque() {
        ...
        userCheckoutPage.selectDropdownByIDAndName(countryBillID, countryBill);
        ...
        userCheckoutPage.selectDropdownByIDAndName(countryDropDownID, newAddress);
        ...
        userCheckoutPage.selectDropdownByIDAndName(countryShipID, countryShip);
    ...
    public void Payment_02_Checkout_Order_Payment_By_Card() {
        userOrderPage.openCategoriesPageByName(ElementData.BasePage.HEADER_CATEGORY, ElementData.BasePage.COMPUTER_CATEGORY, ElementData.BasePage.NOTEBOOKS_SUB_CATEGORY);
        userNotebooksPage = PageGeneratorManager.getUserNotebooksPage(driver);
        userDetailProductPage = userNotebooksPage.clickToDetaiProductByName(appleMacProduct);
        unitPrice = userDetailProductPage.getPriceProduct();
        totalQuantity = userDetailProductPage.getQuantityProductDetail();
        ...
        userCheckoutPage.unclickShipToSameAddressCheckbox();
        userCheckoutPage.selectDropdownByIDAndName(billingAddressID, newAddress);
        userCheckoutPage.inputToTextBoxByID(firstNameBillID, firstNameBill);
        userCheckoutPage.inputToTextBoxByID(lastNameBillID, lastNameBill);
        userCheckoutPage.inputToTextBoxByID(emailBillID, emailBill);
        userCheckoutPage.selectDropdownByIDAndName(countryBillID, countryBill);
    ...
    public void Payment_03_Re_Order() {
        userOrderPage.clickToMenuLinkByName(ElementData.BasePage.MY_ACCOUNT_MENU_LINK_ID);
        userCustomerPage = PageGeneratorManager.getUserCustomerPage(driver);
        userOrderPage = userCustomerPage.clickToOrderPage();
        userOrderPage.clickDetailsButtonByNumBer(orderNumber);
        ...
        verifyEquals(userCheckoutPage.getProductNameCheckout(), appleMacProduct);
        verifyEquals(userCheckoutPage.getDataTableCheckout(unitTableValue), unitPrice);
        verifyEquals(userCheckoutPage.getDataTableCheckout(quantityTableValue), quantity10);
        verifyEquals(userCheckoutPage.getDataTableCheckout(subTotalTableValue), totalBillPrice);
        verifyEquals(userCheckoutPage.getGiftWrappingCheckout(), giftWrap);
        ...
        verifyEquals(userOrderPage.getProductNameOrder(), appleMacProduct);
        verifyEquals(userOrderPage.getDataTableByValue(unitTableValue), unitPrice);
        verifyEquals(userOrderPage.getDataTableByValue(quantityTableValue), quantity10);
        verifyEquals(userOrderPage.getDataTableByValue(subTotalTableValue), totalBillPrice);
        verifyEquals(userOrderPage.getGiftWrappingOrder(), giftWrap);
    ...
    private String radio2ndDay, emailBill, firstNameBill, lastNameBill, countryBill, cityBill, address1Bill,
            ...
            zipCodeShip, phoneShip, moneyMethod, countryDropDownID, appleMacProduct, unitPrice, totalPrice,
            totalQuantity, successCartMess, currentDate, shippingLabel, taxLabel, subTotalPriceLabel, billingAddress,
            nameValue, emailValue, phoneValue, address1Value, cityZipValue, countryValue, shippingAddress,
            firstNameBillID, lastNameBillID, emailBillID, cityBillID, address1BillID, zipCodeBillID, phoneBillID,
            buttonBillID, countryBillID, firstNameShipID, lastNameShipID, emailShipID, cityShipID, address1ShipID,
            zipCodeShipID, phoneShipID, buttonShipID, countryShipID, skuTableValue, unitTableValue, quantityTableValue,
            subTotalTableValue, shippingButtonID, paymentMethodButtonID, paymentInfoButtonID, skuNumber, giftWrap,
            shippingMethod, shipPrice, taxPrice, totalBillPrice, zip550000, newAddress, radio2ndDayLabel, successOrder,
            cardCode, orderNumber, cardMethod, billingAddressID, quantity10, nextDayAirMethod;
